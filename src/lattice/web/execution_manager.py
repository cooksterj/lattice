"""Execution state management for the web service.

This module provides the ExecutionManager that tracks running executions,
connected WebSocket clients, memory usage metrics, and per-asset log
streaming via replay buffers.
"""
...
# These are deferred to avoid pulling in heavy modules at import time:
# - executor types would create a circular dependency (executor -> plan -> registry)
# - observability types would load SQLite and history infrastructure eagerly
if TYPE_CHECKING:
...
def get_memory_snapshot() -> MemorySnapshotSchema:
    ...
    -------
    MemorySnapshotSchema
        Current memory usage snapshot with RSS, VMS, and percent.
    """
    try:
