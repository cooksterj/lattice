...
from lattice.graph.algorithms import _compute_level, _dfs_cycle_detect
...
# registry.py imports from models.py, and graph.py uses AssetRegistry
# only in the ``from_registry`` classmethod type annotation, so we defer
# the import.
if TYPE_CHECKING:
...
class DependencyGraph(BaseModel):
    ...
    def detect_cycles(self) -> list[list[AssetKey]] | None:
        ...
        for node in self.adjacency:
            if color[node] == WHITE:
                _dfs_cycle_detect(node, self.adjacency, color, parent, cycles)
    ...
    def get_execution_levels(self, keys: list[AssetKey] | None = None) -> list[list[AssetKey]]:
        ...
        for key in keys:
            _compute_level(key, self.adjacency, levels, key_set)
